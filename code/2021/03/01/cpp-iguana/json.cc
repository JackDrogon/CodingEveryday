...
void test_tuple()
{
...

	{
		...
		iguana::json::from_json(tp1, json_str.data(),
					json_str.length());
	}
}
...
void compare_speed()
{
	...
	for (auto i = 0; i < Size; ++i) {
		iguana::json::from_json(
			p2, s1.data(),
			s1.length()); //the sequence must be limited
	}
	std::cout << t.elapsed() << '\n';
	t.restart();

	for (auto i = 0; i < Size; ++i) {
		iguana::json::from_json0(
			p2, s2.data(), s2.length()); //no limitation, but slower
	}
	std::cout << t.elapsed() << '\n';
}
...
int main(void)
{
	...
	iguana::json::from_json0(p2, json_str.data(),
	...
	bool bOK = iguana::json::from_json0(
		p3, json_str.data(),
		json_str.length()); //no limitation, but slower
