...
void heap_insert(struct heap *heap, struct heap_node *newnode,
			     heap_compare_fn less_than)
{
	struct heap_node **parent;
	struct heap_node **child;
	...
	/* Calculate the path from the root to the insertion point.  This is a
	 * min
	 * heap so we always insert at the left-most free node of the bottom
	 * row.
	 */
	path = 0;
	...
	/* Now traverse the heap using the path we calculated in the previous
	 * step. */
			     ...
			     heap_compare_fn less_than)
{
	...
	/* Calculate the path from the min (the root) to the max, the left-most
	 * node
	 * of the bottom row.
	 */
	path = 0;
	...
	*max = NULL;
	...
	/* Walk down the subtree and check at each node if the heap property
	 * holds.
	 * It's a min heap so parent < child must be true.  If the parent is
	 * bigger,
